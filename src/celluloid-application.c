...
initialize_gui(CelluloidApplication *app)
{
	...
	*/
	...
	if(video_resolution_index == 0){
		...
		celluloid_view_resize_video_area(view, 853, 480);
	}else{
		celluloid_view_resize_video_area(view, 1278, 720);
	}
	...
	gboolean notif = g_settings_get_boolean(settings, "startup-version-notif-25-12-01");
	if(!notif){
		celluloid_view_show_message_toast(view, "Ecotube updated - See 'How To Use' for details");
		load_default_scripts();
		g_settings_set_boolean(settings, "startup-version-notif-25-12-01", TRUE);
	}
	if(is_modern_osd()){
